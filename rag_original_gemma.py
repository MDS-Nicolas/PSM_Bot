...
# from transformers import CLIPProcessor, CLIPModel # No longer needed if not using CLIP directly for embeddings here
...
class EasyOCRImageLoader:
    def __init__(self, path: str, languages=["en"], device="cpu"):
        self.path = path
...
def call_local_api(messages, max_tokens=200, temperature=0.3):
    ...
    try:
        ...
        print(f"API request failed: {e}. URL: {API_URL}, Data: {json.dumps(data)}")
        raise Exception(f"API request failed: {e}")
    except (KeyError, IndexError) as e:
        print(f"API response format error: {e}. Response: {response.text}")
        raise Exception(f"API response format error: {e}")

# -----------------------------------------------------------------------------
# GPT-4o Image Captioning & Detail Re-query
# -----------------------------------------------------------------------------
def get_image_caption(filepath):
    try:
        prompt = (
            f"File: {filepath}\n"
        ...
        messages = [
            ...
            {"role": "user", "content": f"Image data follows: data:{mime};base64,{b64_str}"}
        ]
        response = call_local_api(messages, max_tokens=200, temperature=0.7)
        return response
    except Exception as e:
        return f"Error describing image via local API: {e}"

def get_image_detail_response(image_path, user_query):
    try:
        ...
        return f"Error retrieving image detail via local API: {e}"

# -----------------------------------------------------------------------------
# Summarize Chat History at Startup
# -----------------------------------------------------------------------------
def summarize_chat_history_at_start(chat_history, chat_file):
    if not chat_history:
        return ""
    ...
    try:
        ...
        print(f"Error summarizing older lines via local API: {e}")
        return chat_history
    summary_lines = [
...
def call_local_model_no_context(query: str):
    try:
        messages = [
...
# -----------------------------------------------------------------------------
def build_final_json_answer(system_prompt: str, context: str) -> QAFormat:
    try:
        json_instruction = (
            "Output **only** valid JSON matching the schema below. "
            "Do not include ```json``` code fences, markdown, or any introductory text. "
...
If no images are relevant, "image_paths" must be an empty list [].

[CONTEXT]
{context}

Based *only* on the CONTEXT, provide the answer in the specified JSON format.
"""
        ...
        response = call_local_api(messages, max_tokens=500, temperature=0.1)
        raw = response.strip()
        ...
        if json_start != -1 and json_end != -1 and json_end > json_start:
            ...
            print(f"Warning: Could not find JSON object in response: {raw}")
        ...
        try:
            ...
            print(f"Warning: JSONDecodeError parsing response: {e}. Raw response used as answer: {raw}")
            return QAFormat(answer=raw, image_paths=[])
        except Exception as pydantic_e:
             print(f"Warning: Pydantic validation error: {pydantic_e}. Raw response used as answer: {raw}")
             return QAFormat(answer=raw, image_paths=[])

    except Exception as e:
        print(f"Error in build_final_json_answer: {e}")
...
class RAG:
    def __init__(self, 
        ...
        self.chat_history = summarize_chat_history_at_start(self.chat_history, self.CHAT_FILE)

        # Initialize databases
        self.psm_db = self.build_or_load_psm_db(force=reload_psm)
        self.clip_db = self.build_or_load_image_db(force=reload_img)
        self.clip_embedder = CLIPImageEmbeddings(device=self.device) if self.clip_db else None
    ...
    def build_or_load_psm_db(self, force=False):
        ...
        if changed:
            ...
            docs_imgs = []
            print(f"Processing {len(imgs)} images for text DB...")
            for i, ip in enumerate(imgs):
                print(f"  Processing image {i+1}/{len(imgs)}: {ip}", end='\r')
                try:
                    loader = EasyOCRImageLoader(ip, ["en"], device=self.device)
                    docs_imgs.extend(loader.load())
                except Exception as e:
                    print(f"\nError loading/processing image {ip}: {e}")
    ...
    def Get_Rag_Response(self, user_q: str) -> dict:
        ...
        for doc in psm_hits:
            if needs_image_requery(doc, user_q, self.IMAGE_KEYWORDS):
                detail = get_image_detail_response(doc.metadata["source"], user_q)
        ...
        structured_answer = build_final_json_answer(final_system_prompt, combined_context)

        if is_generic(structured_answer.answer):
            print("INFO: RAG answer was generic, attempting fallback...")
            try:
                fb = call_local_model_no_context(user_q)
                fb = augment_fallback(user_q, fb)
                if image_details:
                    fb += "\n\nAdditional Image Details:\n" + "\n".join(image_details)
                structured_answer.answer = fb
                structured_answer.image_paths = []
                approach = "fallback (local model)"
                print(f"INFO: Fallback successful. Approach: {approach}")
            except Exception as e:
                 print(f"ERROR: Fallback call failed: {e}")
                 structured_answer.answer = f"I couldn't find specific information in the documents. (Fallback failed: {e})"
                 structured_answer.image_paths = []
                 approach = "RAG (generic, fallback failed)"

        else:
            approach = "RAG (local model)"
        ...
        ai_response_json = structured_answer.json()
        self.session_memory.chat_memory.add_ai_message(ai_response_json)
        self.chat_history += f"Assistant: {ai_response_json}\n"

        # Write assistant response to chat_history.txt
        try:
            with open(self.CHAT_FILE, "a", encoding="utf-8") as f:
                f.write(f"Assistant: {ai_response_json}\n")
        except Exception as e:
